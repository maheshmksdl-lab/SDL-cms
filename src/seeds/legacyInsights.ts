/**
 * Run by `pnpm seed` after the design seed, which creates the services, products and categories
 * these articles are tagged with — and on its own by `pnpm migrate:insights`.
 *
 * Originally a one-time content migration: brings Blog / Case study / Whitepaper Insights across from the
...
const DATA_DIR = path.resolve(__dirname, '../../scripts/migrate-insights-data')
...
export async function seedLegacyInsights(payload: Payload): Promise<void> {
