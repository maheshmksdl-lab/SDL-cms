/**
 * One-time content migration: brings the real client logos shown on the legacy site's
 * https://www.socialdnalabs.com/clients/ ("Trusted by Industry Leaders" grid — every logo on
 * that page, confirmed by diffing the full set of <img> tags in that section against the page's
 * total image count) into the `clients` collection, replacing the 6 generic placeholder logos
 * (`Client 1`…`Client 6`, seeded from design mockups — see scripts/migrate-insights-data's sibling
 * note in memory) that were seeded before any real logo assets existed.
 *
 ...
 * are upserted by name (existing docs updated in place, not duplicated). The 6 placeholder
 * records are only deleted if they still match their original seeded shape exactly (name
 * "Client N" + logo filename "clientN.jpg") — if someone has already customized one via the
 * admin, this script leaves it alone rather than guessing.
 */
...
const LOGOS_DIR = path.join(__dirname, 'migrate-clients-data/logos')
...
const PLACEHOLDER_NAMES = new Set(['Client 1', 'Client 2', 'Client 3', 'Client 4', 'Client 5', 'Client 6'])
...
async function removeStalePlaceholders(payload: Payload): Promise<number> {
  const existing = await payload.find({
    collection: 'clients',
  ...
  let removed = 0
  for (const doc of existing.docs as { id: number; name: string; logo?: { filename?: string } }[]) {
    const expectedFilename = `client${doc.name.replace('Client ', '')}.jpg`
    const actualFilename = doc.logo?.filename
    if (actualFilename !== expectedFilename) {
      console.log(`  · leaving "${doc.name}" alone — logo is "${actualFilename}", not the original placeholder "${expectedFilename}" (looks edited)`)
      continue
    }
    console.log(`  · removing placeholder "${doc.name}" (${actualFilename})`)
    if (!DRY_RUN) await payload.delete({ collection: 'clients', id: doc.id, overrideAccess: true })
    removed += 1
  }
  return removed
}
...
async function main() {
  ...
  console.log('')
  const removed = await removeStalePlaceholders(payload)

  console.log(
    `\n  ${DRY_RUN ? 'would create' : 'created'}: ${created}   ${DRY_RUN ? 'would update' : 'updated'}: ${updated}   ${DRY_RUN ? 'would remove' : 'removed'} placeholders: ${removed}\n`,
  )
  process.exit(0)
}
